...
export class HospiCashQuotesComponent implements OnInit {
...
private buildPayloadFromLocal(ls: any): PlanPayload {
  console.log('HC LocalStorage Data:', ls);

  // ✅ Hospicash enquiry stores room rent, but backend needs coverAmount
  const coverAmount = this.toNum(ls.room_rent ?? ls.coverAmount ?? ls.cover_amount ?? 0, 0);

  const age = this.toNum(ls.Age ?? ls.age ?? 0, 0);

  const noOfDays = this.toNum(ls.no_of_days ?? ls.noOfDays ?? 0, 0);

  // If you don't have these in enquiry, keep safe defaults
  const noOfAdults = this.toNum(ls.noOfAdults ?? ls.no_of_adults ?? 1, 1);
  const noOfChildren = this.toNum(ls.noOfChildren ?? ls.no_of_children ?? 0, 0);
